...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <div class="card-title fw-mediumbold">Редактирование счета "{{$invoice->number_invoice}}"</div>
                    <form action="{{route('invoices.update', ['invoice' => $invoice->id])}}" class="mt-3" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="form-group @error('supplier_id') has-error @enderror">
                            ...
                            <select class="js-example-basic-single form-control" name="supplier_id" id="supplier_id">
                                ...
                                @foreach($suppliers as $k => $v)
                                    <option {{ $invoice->supplier_id == $k ? 'selected' : '' }}
                                            value="{{$k}}">{{$v}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group @error('number_invoice') has-error @enderror">
                            ...
                            <input type="text" class="form-control" name="number_invoice" id="number_invoice" placeholder="Введите номер счета..." value="{{$invoice->number_invoice}}">
